#Adapted from Flowcytometry lib written by Sarah Goldberg
...
module Cytometers
  ...
  module Cytometer
    ...
    def run_sample_96(args)
      ...
      run_template(template_file: args[:templates][sample_string],
                   ...
                   sample_matrix: samp_matrix,
                   item_number: coll.id)
      uploads = upload_files(expected_uploads: 1)
      ...
      associate_uploads_to_plate(KEY_SAMPLE.pluralize, coll, uploads) if !debug # associate a matrix of uploads to the plate
    ...
    def run_defaults
      {
        templates: { 'E coli' => 'Ecoli.c6t', 'Yeast' => 'SynAg_Display.c6t' },
        container: '96 well plate: U-bottom (Clear)'
      }
    end
    ...
    private def uploads_from_show_to_array(uploads_from_show, upload_var)
      ...
      return [] if uploads_from_show[upload_var].nil?
      upload_list = []
  ...
  class BDAccuri
    ...
    CYTOMETER_NAME = 'BD Accuri'.freeze
    TEMPLATE_DIR = 'aq_templates'.freeze
    ...
    def run_settings
      {
        settings: {
          'E coli' => { 'Run Limits' => '60,000 events, 1 Min, 50 µL',
                        'Fluidics' => 'Medium',
