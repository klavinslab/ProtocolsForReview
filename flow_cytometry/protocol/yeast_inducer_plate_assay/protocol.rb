...
DESTINATION_FOLDER = 'Documents > BIOFAB_Yeast_Inducer_Plate_Assay'
  ...
  def main
    ...
    operations.each do |op|
        ...
        well_summary = rows_in_use.map{|r| "Row " + r + ", wells 1 - 8"}
        ...
        yeast_item_table = Table.new(
              ...
              d: "Volume of yeast overnight (l)"
            )
            
        items.each do |i|
            yeast_item_table.a(rows_in_use[items.index(i)]).b("1 - 8").c(i).d("25").append
        end
       ...
       ######################################### 
        ...
        inducer_wells.each do |i|
            inducer_transfer_table.a(i).b("1 uL").c(transfer_wells[i - 1]).append
        end
        ...
        show do 
            ...
            check "Save file in Folder #{DESTINATION_FOLDER} as 'Yeast Inducer Plate - #{op.user.name} - #{Time.now.strftime("%m/%d/%Y")}"
        end
