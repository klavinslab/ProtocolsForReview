    ...
    def main
        ...
        show do
            ...
            table operations.start_table
              .input_item(INPUT_P)
              .get(:vol_P, type: 'number', heading: "Plasmid Volume (L)", default: 0)
              .input_item(INPUT_F)
              .get(:vol_F, type: 'number', heading: "Insert Volume (L)", default: 0)
              .end_table 
        end 
        ...
        show do
            ...
            table operations.start_table
              .output_item(OUTPUT)
              .input_item(INPUT_P)
              .custom_column(heading: "Plasmid volume (L)", checkable: true) { |op| (op.input(QUANTITY_P).val.to_f*1000/op.input(INPUT_P).item.get(:concentration).to_f).round(2) }
              .input_item(INPUT_F)
              .custom_column(heading: "Insert volume (L)", checkable: true) { |op| (op.input(QUANTITY_F).val.to_f*1000/op.input(INPUT_F).item.get(:concentration).to_f).round(2) } 
              .end_table
        ...
        show do
            ...
            note "Weigh combined samples <b>#{finished.map { |op| op.output(OUTPUT).item.id}.to_sentence}</b> and enter volumes into the following table (1mg=1L):"
            table operations.start_table
                .output_item(OUTPUT)
                .get(:final_volume, type: 'number', heading: "Final Volume (L)", default: 0) 
                .end_table
        end
