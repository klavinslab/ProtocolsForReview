...
module LibraryCloningHelper
    ...
    def estimateVolumes(ops, inOrOut, ioStr, max_vol, nloops)
        ...
        case inOrOut
            when 'in'
                ...
                tab = ops.start_table
                    .input_item(ioStr)
                    .custom_input(:high_vol, heading: "Contains more than #{max_vol} µL ?", type: "string") {|op| op.temporary[:high_vol] || 'N'}
                    .end_table
            when 'out'
        ...
        show do
            ...
            note "For sample(s) <b>samples</b>, enter <b>Y</b> if sample is over #{max_vol} µL (indictaed by black line on tube):"
            table tab
            note "If there is no line on the tube, compare to #{max_vol} µL in an empty tube (labeled <b>MG</b>)"
        end
    end
    ...
    def prep_speedvac_tubes(ids,max_vol)
        ...
        show do
            ...
            check "Transfer #{max_vol} µL of MG water into one of the tubes and mark its lid <b>MG</b>"
            check "Label empty tubes for the combined samples: <b>#{ids.to_sentence}</b>"
            check "Mark the #{max_vol} µL level of the labeled tubes (using a sharpee) by comparison to the tube labeled <b>MG</b>"
