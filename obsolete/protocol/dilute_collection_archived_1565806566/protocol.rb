...
class Protocol
  ...
  def main
    ...
    operations.each do |op|
      ...
      op = (debug ? Operation.find(177881) : op)
      ...
      op.input_array(INPUT).collections.each_with_index do |in_collection, idx|
        ...
        gather_materials(empty_containers: output_diluted_plates, new_materials: ['Multichannel Pipette', 'Media Reservoir'], take_items: [])
  ...
  def pre_fill_collection(out_collection:, media_hash:, part_associations_matrix:, transfer_vol_matrix:)
    ...
    part_associations_matrix.each_with_index do |row, r_i|
      row.each_with_index do |part, c_i|
        if !part.empty?
          media_name = part.fetch("Media").keys.first
          position = [r_i, c_i]
          if media_position_hash.keys.include? media_name
            media_position_hash[media_name].push(position)
          else
            media_position_hash[media_name] = [position]
          end
        end
      end
    end
