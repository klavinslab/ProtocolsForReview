...
class Protocol
  ...
  def main
    ...
    operations.each do |op|
      ...
      op = Operation.find(211577) if debug
    ...
    clean_up(item_arr: clean_up_array.flatten)
  end # main
  ...
  def get_item_id_to_rc_list(part_data_matrix:, experimental_condition:)
    ...
    #   available_option_keys = part_data_matrix.map {|row| row.map {|part| part.fetch(experimental_condition, {}) } }.flatten.uniq.reject{|part| part.empty? }.map {|part| part.keys.first}.uniq 
      available_option_keys = part_data_matrix.map {|row| row.map {|part| part.fetch("Options", {}) } }.flatten.uniq.reject{|part| part.empty? }.map {|part| part.keys.first}.uniq 
      option_key = show do
