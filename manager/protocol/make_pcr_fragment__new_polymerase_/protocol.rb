...
class Protocol
  ...
  # other
  SEC_PER_KB = 60 # sec, extension timer per KB for KAPA
  ENZYME_NAME = "" # Sample name of the type of polymerase to use for reactions
  ENZYME_AMOUNT = 25 # uL
  ...
  def main
    ...
    master_mix = "ALTERNATE POLYMERASE"#find(:sample, name: ENZYME_NAME)[0].in("Enzyme Stock")[0]
    # take [master_mix], interactive: true,  method: "boxes"
    ...
    add_mix stripwell_tab, master_mix
    ...
    # release [master_mix], interactive: true
  ...
  # add kapa master mix to stripwells
  def add_mix stripwell_tab, master_mix
      show do
          title "Add Master Mix"
          
          note "Pipette #{ENZYME_AMOUNT} uL of master mix (#{master_mix}) into stripwells based on the following table:"
          table stripwell_tab
