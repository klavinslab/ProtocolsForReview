...
class Protocol
  ...
  def main
    ...
    operations.sort! { |op1, op2| op1.input(INPUT).item.id <=> op2.input(INPUT).item.id }
    operations.make
    
    raise "some operation's output had no item" if (operations.select {|op| op.output(OUTPUT).item.nil?}).any?
    
    # While testing, assign a random weight value
    operations.each{ |op| op.set_input_data(INPUT, :weight, Random.rand / 2 + 0.1)  } if debug
    
    operations.each do |op|
        if op.input(INPUT).sample_type.name == "DNA Library"
            
            op.temporary[:qg_volume]  = (op.input_data(INPUT, :weight).to_f / DENSITY1).floor
            op.temporary[:total_volume] = op.temporary[:qg_volume]
            op.temporary[:is_divided] = op.temporary[:total_volume] >= 2000        
        else
            # reached here
            # might need to add checks if a field isnt defined in the sample
            op.temporary[:qg_volume]  = (op.input_data(INPUT, :weight).to_f / DENSITY1).floor
            op.temporary[:iso_volume] = (op.input_data(INPUT, :weight).to_f / DENSITY2).floor
            op.temporary[:iso_volume] = 0 if op.input(INPUT).sample.properties["Length"].between?(500, 4000)
    ...
    operations.each do |op|
      ...
      op.set_output_data(OUTPUT, :concentration, op.temporary[:conc])
      if op.output(OUTPUT).item != nil
        op.output(OUTPUT).item.notes =  op.temporary[:note]
      end
    end
...
#   end
  ...
  def get_choices
    choices = nil
    if !operations.select{ |op| op.output_data(OUTPUT, :concentration) < 10}.empty?
        choices = show do
          title "Decide whether to keep dilute stocks"
          note "The below stocks have a concentration of less than 10 ng/uL."
          note "Talk to a lab manager to decide whether or not to discard the following stocks."
          operations.select{ |op| op.output_data(OUTPUT, :concentration) < 10}.each do |op|
            select ["Yes", "No"], var: "d#{op.output(OUTPUT).item.id}", label: "Discard Fragment Stock #{op.output(OUTPUT).item.id}", default: 1
          end
        end #if operations.any?{ |op| op.output_data("Fragment", :concentration) < 10}
    end
  ...
  # This method displays a table of gels and tells the technician to add PB buffer
  ...
  def add_isopropanol
   ...
   def centrifuge
    show do
      ...
      warning "Add the remaining PB mixtures to their corresponding columns, and repeat these first two steps for all tubes with remaining mixture!"
   ...
   def transfer_to_tube
    show do
        ...
        table operations.start_table
            ...
            .output_item(OUTPUT, heading: "1.5 mL tube", checkable: true)
        .end_table
        check "Add 30 uL molecular grade water to center of the column."
   ...
   def discard_fragment_stocks choices
     if !choices.nil? && choices.any? { |key, val| val == "Yes"}
      show do
        ...
        note operations.select{ |op| choices["d#{op.output(OUTPUT).item.id}".to_sym] == "Yes"}
          .map{ |op| op.output(OUTPUT).item.id}
          .join(", ")
      end
     end
   end
