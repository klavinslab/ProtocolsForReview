...
class Protocol
  ...
  def main
    ...
    raise "some operation's output had no item" if (operations.select {|op| op.output("Fragment").item.nil?}).any?
    
    # While testing, assign a random weight value
    operations.each{ |op| op.set_input_data("Gel", :weight, Random.rand / 2 + 0.1)  } if debug
    
    operations.each do |op|
        if op.input("Gel").sample_type.name == "DNA Library"
    ...
    operations.each do |op|
      ...
      if op.output("Fragment").item != nil
...
#   end
  ...
  def get_choices
    choices = nil
    if !operations.select{ |op| op.output_data("Fragment", :concentration) < 10}.empty?
        choices = show do
          title "Decide whether to keep dilute stocks"
          note "The below stocks have a concentration of less than 10 ng/uL."
          note "Talk to a lab manager to decide whether or not to discard the following stocks."
  ...
  def heat_block
    show do
      ...
      check "Start timer."
      check "Vortex every few minutes to speed up the process."
      check "Retrieve after 10 minutes or until the gel slice is competely dissovled."
    end
  end
  ...
  def add_isopropanol
   ...
   def use_label_printer
    show do
      ...
      if operations.first.output("Fragment").item != nil
   ...
   def measure_DNA
    show do
      ...
      check "Nanodrop each fragment and enter the concentration below."
