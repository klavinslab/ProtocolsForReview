...
class Protocol
	include StripwellMethods, Feedback
	...
	def main
        ...
        plate.associate :num_qc_colonies, plate.get(:num_colonies).to_i + 1 if plate.created_at < Time.utc(2018, "jan", 11, 19, 15)
         ...
         if !plate.sample.properties["QC Primer1"] && !plate.sample.properties["QC Primer2"]
            fwd = Sample.find_by_id(5606).properties["QC Primer1"]
            rev = Sample.find_by_id(5606).properties["QC Primer2"]
        elsif !plate.sample.properties["QC Primer1"]
        ...
        elsif !plate.sample.properties["QC Primer2"]
        ...
        sw = []
            ...
            ops.output_collections["Lysate"].each do |sw|
             ...
             note "For each plate, you will mark (if the colony isn't already marked) and pick colonies."
             table ops.start_table
                .custom_column(heading: "Well") { |op| op.output("Lysate").column + 1 }
                .custom_column(heading: "Colony cx from plate, 1/3 size", checkable: true) { |op| "#{op.input("Plate").item.id}.c#{op.temporary[:colony]}" }
             .end_table
             note "For each plate id.cx (x = 1,2,3,...), if a colony is not already marked with <b>cx</b>, mark a random colony with a circle and write <b>c<i>x</i></b> ('c1', 'c2', 'c3', etc) nearby. If the colony is already marked with that <b>cx</b> label on the plate, just scrape that colony."
             note "Use a sterile 10 L tip to scrape about 1/3 of the marked colony, swirl tip inside the corresponding well until mixed."
         end
     end
    ...
    #   Run the thermocycler
            ...
            operations.store io: "input", interactive: true
             ...
             show do
                ...
                # warning "DO NOT SPIN DOWN STRIPWELLS."  # why? The cells should be lysed. In fact, spinning it would be better for the colony PCR.
            end
