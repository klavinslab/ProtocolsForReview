...
# September 26, 2018
# Based on Transform protocol by Ayesha Saleem
...
class Protocol
  ...
  def display_table index
    show do
      ...
      table operations.running.start_table 
        ...
        .custom_column(heading: "Electroporator Voltage") { |op| op.input("Voltage").val}
  ...
  def incubate_transformants 
    show do 
      ...
      time = operations[0].input("Incubation Length").val.round
      note "Transformants should incubate for #{time} minutes"
      note "<a href=\'https://www.google.com/search?q=#{time}%20minute%20timer\' target=\'_blank\'>Use a #{time} minute Google timer</a> to set a reminder to retrieve the transformants, at which point you will start the \'Plate Transformed Cells\' protocol."
  ...
  def plate_preheating  
    show do 
      ...
      note "Retrieve the following plates, and place into still incubator."    
      heated_plates = operations.running.find_all { |op| op.input("Plate Preheating") == "Yes" }
      ...
      note "Place plates for ONLY the following samples into the still #{operations[0].input("Plasmid").sample.properties["Transformation Temperature"].to_i} C incubator"
      ops_for_heating = operations.running.select { |op| op.input("Preheating") == "Yes" }
      table ops_for_heating.start_table
          .input_item(INPUT, heading: "Sample ID", checkable: true)
          .custom_column(heading: "Bacterial Marker", checkable: true) { |op| "#{op.input(INPUT).sample.properties["Bacterial Marker"]}" }
          .end_table
