...
class Protocol
  ...
  def main
    ...
    # replaces operations.store. groups operations by input/output and stores them
    input_store = {}
    output_store = {}

    operations.each do |op|
        input_store["Item #{op.input("Plasmid").item} (#{op.input("Plasmid").item.object_type.name})"] = op.input("Plasmid").item.location if op.input("Plasmid").item.location != 'deleted'
        output_store["Item #{op.output("Overnight").item} (#{op.output("Overnight").item.object_type.name})"] = op.output("Overnight").item.location if op.output("Overnight").item.location != 'deleted'
        
    end
    
    show do
        title "Return the Following Item(s) "
        input_store.each do |k, v|
            check "#{k} at #{v}"
        end

        separator
        output_store.each do |k, v|
            check "#{k} at #{v}"
        end
    end
