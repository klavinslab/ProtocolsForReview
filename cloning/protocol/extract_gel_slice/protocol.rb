...
class Protocol
    ...
    def main
      ...
      grouped_gels = operations.group_by{ |op| op.input(FRAGMENT).collection}
      ...
      gels.each { |gel|
        ...
        if(grouped_ops.any?)
          ...
          # weigh fragments, == 1 because image -> cut -> weight if theres only one gel
          weigh_fragments grouped_ops if grouped_gels.length == 1
          ...
          grouped_ops.each { |op|
            ...
            # == 1 because image -> cut -> weight if theres only one gel
            op.output(FRAGMENT_OUT).item.associate(:weight, op.temporary[:weight]) if grouped_gels.length == 1 
          }
      ...
      # > 1 because image -> cut -> clean -> weigh ALL if theres more than one gel
      if (grouped_gels.length > 1)
          weigh_fragments operations
