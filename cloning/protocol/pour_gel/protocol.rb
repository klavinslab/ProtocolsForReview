...
class Protocol
  ...
  def top_bottom_combs gels_needed
    show do
      title "Set up gel box for #{gels_needed} gels"
      
      check "You will need #{gels_needed} gels."
  ...
  def add_gel_green
    show do
      title "Add gel green to each gel"
    #   note "Using a 10 µL pipetter, take up 5 µL of GelGreen into the pipet tip. Expel the GelGreen directly into the molten agar (under the surface), then swirl to mix."
      check "Using a 10 µL pipette, take up 5 µL of GelGreen into the pipette tip."
      check "Expel the GelGreen directly into the molten agar (under the surface), then swirl to mix."
  ...
  def pour_label_gels
    show do
      ...
      operations.output_collections["Lane"].each_with_index do | gel, i |
          check "Write id #{gel.id} the date, and your initials on piece of lab tape and affix it to the side of the gel box."
      end
