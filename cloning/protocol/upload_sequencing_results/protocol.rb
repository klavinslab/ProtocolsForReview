...
class Protocol
  ...
  def results_have_arrived?(tracking_num)
    results_info = show do
      ...
      note "Under 'My Orders' select the 'View Results' button for the order with tracking number #{tracking_num} and check if the sequencing results have shown up yet"
  ...
  def upload_batched_results(tracking_num)
    show do
      ...
      check "Select all of the sequencing results, then select '.ab1', and click 'download'"
  ...
  def upload_individual_results(tracking_num)
    ...
    5.times do
      ...
      sequencing_uploads = show do
        ...
        check "Navigate to your downloads folder." 
        check "Select the recently downloaded folder with number #{tracking_num}"
        check "Double click on the zipped folder, then double click on the folder within, highlight all of the sequencing results from here then drag and drop them into your downloads folder."
        check "Click the button below and upload all of the files from the table below."
